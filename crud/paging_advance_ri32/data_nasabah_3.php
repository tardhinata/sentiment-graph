...
<tr>
	...
	<input type="hidden" value="data_nasabah_3" name="page">
...
if((isset($_GET['cari']))){
	...
	if(empty($nasabah)){
		...
		//untuk paging dinamis
		$url_cari="&nasabah=$nasabah&tanggal_awal=$tanggal_awal&tanggal_akhir=$tanggal_akhir&cari=Tampilkan+Data";
		$pencarian="where nama_nasabah like '%$nasabah%' and tanggal between '$tanggal_awal' and '$tanggal_akhir'";
	}
...
$query 	= "SELECT * FROM tabel_nasabah $pencarian LIMIT $offset, $dataPerhalaman";
...
$query  = "SELECT COUNT(*) AS jumData FROM tabel_nasabah $pencarian";
...
if ($nohalaman > 1){
	echo  "<a href='".$_SERVER['PHP_SELF']."?page=data_nasabah_3&halaman=".($nohalaman-1)." $url_cari'>&lt;&lt; Prev</a>";
}
...
for($halaman = 1; $halaman <= $jumhalaman; $halaman++)
{
		 ...
		 {   
			...
			if ($halaman == $nohalaman){ 
				...
				echo " <a href='".$_SERVER['PHP_SELF']."?page=data_nasabah_3&halaman=".$halaman." $url_cari'>".$halaman."</a> ";
			}
			$showhalaman = $halaman;          
		 }
}
...
if ($nohalaman < $jumhalaman){ 
	echo "<a href='".$_SERVER['PHP_SELF']."?page=data_nasabah_3&halaman=".($nohalaman+1)." $url_cari'>Next &gt;&gt;</a>";
}
?>
</font>
