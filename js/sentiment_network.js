...
function D3ok(_projectName, _sentiment) {
	...
	var json_url = 'graph/graph_generator.php?projectName='+projectName+'&sentiment='+sentiment;
	d3.json( json_url, //'backend/data.json',
		function (data) {
			...
			*/
			...
			var graphLinks = networkGraph.append("svg:g").selectAll("path")
		    .data(linkArray, function (d) {
					return d.source.index + '-' + d.target.index;
				})
		  .enter().append("path")
			...
			/* --------------------------------------------------------------------- */
			/* Perform zoom. We do "sentiment zoom", not geometric zoom
